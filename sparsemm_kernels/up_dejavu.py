...
def sparsemm_up_dejavu(
    A, B, C, IDX, ACT_TYPE="fatrelu", tune=False,
    BLOCK_SIZE_M=None, BLOCK_SIZE_K=None, BLOCK_SIZE_Q=None,
    ...
    sparsemm_up_dejavu_kernel[grid](
        A, B, C, D, IDX,
        M, N, K, P, Q,
        ...
        BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_Q, ACT_TYPE,
        num_stages=num_stages, num_warps=num_warps
    )
    return D

def bench_sparsemm_up_dejavu(
    ...
    try:
        ms = triton.testing.do_bench(lambda: sparsemm_up_dejavu(
            A, B, C, IDX, ACT_TYPE, True,
            BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_Q,
            num_stages, num_warps
        ))
        return ms
    except Exception as e:
        logger.warning(f"Error in bench_sparsemm_up_dejavu: {e}")
        return float("inf")